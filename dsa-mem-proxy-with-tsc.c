...
static int rdmsr(int msr, uint64_t *val)
{
    int fd;
    int ret;
...
{
	uint64_t hz = 0;
	int32_t ret = rdmsr(0xCE, &hz);
	if (ret < 0)
		return 0;
	else
		return ((hz >> 8) & 0xff) * 100 * 1E6;
}
...
static void update_stats(int op, size_t n, uint64_t elapsed_ns, int group)
{
	if (unlikely(!collect_stats))
		return;
	int bucket = ((n+1) % HIST_BUCKET_SIZE);
...
static void init_mem_proxy(void)
{
	...
	if (atomic_compare_exchange_strong(&dsa_initialized, &init_notcomplete, 1)) {
		...
		int32_t ret = rdmsr(0xCE, &tsc_freq);
		if (ret >= 0)
			tsc_freq = ((tsc_freq >> 8) & 0xff) * 100 * 1E6;
...
#if 0
			if (op == MEMCOPY) {
				printf("%d %d\n", TS_NS(st, et), ((er-sr)*1000000000)/tsc_freq);
			}
#endif
			update_stats(op, n, collect_stats ? TS_NS(st, et) : 0, DSA_CALL_SUCCESS);
			//update_stats(op, n, collect_stats ? (((er-sr)*1000000000)/tsc_freq) : 0, DSA_CALL_SUCCESS);
		}
	}
